...
fn process_instruction(program_id: &Pubkey,accounts: &[AccountInfo],instruction_data: &[u8],) -> ProgramResult { 
    ...
    if  instruction_data[0] == 0 {
        ...
        return donate(program_id,
        accounts;
        &instruction_data[1..instruction_data.len()];)
    }

    msg!("Didn't found the entry point required");
...
fn create_campaign(
    ...
    let mut input_data = CampaignDetails::try_from_slice(&instruction_data).expect("Instruction data serialization didn't worked");
...
fn withdraw(
    ...
    let input_data = WithdrawRequest::try_from_slice(&instruction_data).expect("Instruction data serialization didn't worked");
